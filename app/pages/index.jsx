...
import mk, { renderPendingKatex } from './katex'
...
// Debug: verify macros loaded
if (typeof window !== 'undefined') {
  console.log('KaTeX macros loaded:', Object.keys(katexMacros).length, 'macros')
  console.log('Sample macros:', Object.keys(katexMacros).slice(0, 10))
}
...
export default class PreviewPage extends React.Component {
  ...
  componentDidMount() {
    ...
    // Render any pending KaTeX blocks (KaTeX is loaded synchronously via script tag)
    renderPendingKatex(DEFAULT_OPTIONS.katex);
  }
    ...
    const refreshRender = () => {
      this.setState({
        ...
        if (refreshContent) {
          ...
          // Render KaTeX math (merge with default options including macros)
          renderPendingKatex({ ...DEFAULT_OPTIONS.katex, ...(options.katex || {}) })
        }
        refreshScroll()
      })
    }
    ...
    return (
      <React.Fragment>
        <Head>
          ...
          <script type="text/javascript" src="/_static/preview-persistence.js"></script>
        </Head>
